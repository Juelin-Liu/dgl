...
IdArray MetisPartition(
    NDArray indptr, NDArray indices, int k, NDArray vwgt_arr, NDArray ewgt_arr, const std::string &mode,
    bool obj_cut) {
  // Mode can only be "k-way" or "recursive"
  CHECK(mode == "k-way" || mode == "recursive")
      << "mode can only be \"k-way\" or \"recursive\"";
  ...
  idx_t e_num = indices.NumElements();
  idx_t ncon = 1;  // # balacing constraints.
  idx_t *xadj = static_cast<idx_t *>(indptr->data);
  idx_t *adjncy = static_cast<idx_t *>(indices->data);
  idx_t nparts = k;
  idx_t objval = 0;
  ...
  int64_t vwgt_len = vwgt_arr->shape[0];
  int64_t ewgt_len = ewgt_arr->shape[0];

  CHECK_EQ(sizeof(idx_t), vwgt_arr->dtype.bits / 8)
      << "The vertex weight array doesn't have right type";
  CHECK_EQ(sizeof(idx_t), ewgt_arr->dtype.bits / 8)
      << "The edge weight array doesn't have right type";
  ...
  idx_t *ewgt = NULL;
  if (ewgt_len > 0) {
    ewgt = static_cast<idx_t *>(ewgt_arr->data);
  }
  ...
  if (obj_cut) {
    LOG(INFO) << "Partition a graph with " << nvtxs << " nodes and "
              << e_num << " edges into " << k << " parts and "
              << "get " << objval << " edge cuts";
  } else {
    LOG(INFO) << "Partition a graph with " << nvtxs << " nodes and "
              << e_num << " edges into " << k << " parts and "
...
DGL_REGISTER_GLOBAL("partition._CAPI_DGLMetisPartitionEdge_Hetero")
    .set_body([](DGLArgs args, DGLRetValue *rv) {
      ...
      NDArray vwgt = args[3];
      NDArray ewgt = args[4];
      std::string mode = args[5];
      bool obj_cut = args[6];
      if (ewgt.NumElements() != 0) CHECK_EQ(indices.NumElements(), ewgt.NumElements());

#if !defined(_WIN32)
      *rv = MetisPartition(indptr, indices, k, vwgt, ewgt, mode, obj_cut);
#else
      LOG(FATAL) << "Metis partition does not support Windows.";
#endif  // !defined(_WIN32)
    });
