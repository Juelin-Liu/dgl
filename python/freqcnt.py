...
def get_edge_freq(graph: dgl.DGLGraph, train_idx: torch.Tensor):
    v_num = graph.num_nodes()
    e_num = graph.num_edges()
    device = 0    
    graph_sampler = dgl.dataloading.NeighborSampler(fanouts=[20, 20, 20])
    ...
    step = 0
    epoch_num = 10
    eflag = torch.zeros(e_num, dtype=torch.bool).to(device)
    efreq = torch.zeros(e_num, dtype=train_idx.dtype).to(device)
    ...
    print(f"duration = {timer.duration()} s")
    return eflag.to("cpu"), efreq.to("cpu")

def freqcnt(data_dir: str, graph_name: str, num_partitions = 4):
    in_dir = os.path.join(data_dir, graph_name)
    graph: dgl.DGLGraph = load_dgl_graph(in_dir, is32=True, wsloop=False)
    ...
    eflag, efreq = get_edge_freq(graph, train_idx)
    torch.save(eflag, os.path.join(cur_dir, f"freqcnts/{graph_name}_eflag.pt"))
    torch.save(efreq, os.path.join(cur_dir, f"freqcnts/{graph_name}_efreq.pt"))
...
dataset = {
    # "ogbn-products":   "/mnt/homes/juelinliu/dataset/OGBN/processed",
    # "ogbn-papers100M": "/mnt/homes/juelinliu/dataset/OGBN/processed",
    # "com-orkut": "/mnt/homes/juelinliu/dataset/snap/",
    "com-friendster": "/mnt/homes/juelinliu/dataset/snap/"
}

for graph_name, data_dir in dataset.items():
    freqcnt(data_dir, graph_name)
