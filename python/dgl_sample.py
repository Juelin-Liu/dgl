...
def bench(configs: list[Config], test_acc=False):
    ...
    for config in configs:
        # Default settings
        try:
            train(rank, config, graph, train_idx, test_idx, valid_idx, feat, label)
...
def train(rank: int, config: Config, graph: dgl.DGLGraph, train_idx: Tensor, test_idx, valid_idx, feat, label):
    graph_sampler = dgl.dataloading.NeighborSampler(fanouts=config.fanouts, replace=config.replace)
    dataloader = dgl.dataloading.DataLoader(
        graph=graph,               # The graph
        indices=train_idx.to(rank),         # The node IDs to iterate over in minibatches
